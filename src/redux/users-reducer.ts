...
let initialState = {
    ...
    currentPage: 1,
    isFetching: true,
    followingWithProgress: [] as Array<number> // array of users ids
};
...
const usersReducer = (state = initialState, action: any): InitialStateType => {
    switch (action.type) {
        ...
        case TOGGLE_IS_FOLLOWING_PROGRESS:
            return {
                ...state,
                followingWithProgress: action.isFetching
                    ? [...state.followingWithProgress, action.userId]
                    : state.followingWithProgress.filter(id => id !== action.userId)
            };
        default:
            return state;
    }
